# `HumanRail999` - <기차표 예매 사이트(4회차 프로젝트)>🚂
1. 작업 기간 : 2024. 06. 19 ~ 2024. 07. 12
2. 주제 : 기차표 예매 사이트
3. 목적 : 고객이 온라인에서 HumanRail999 사이트를 통하여 기차 배차시간 확인, 기차표 예매 등의 편리한 기차 서비스 이용을 주목적으로 진행된 기차표 예매 사이트 프로젝트 입니다.<br>
...
-단, 필수 약관 내용에 동의한 경우에만, 회원정보 등록 페이지로 이동된다(조건). <br>
...
-단, 아이디 중복확인과 비밀번호 재확인에 성공한 경우에만, 회원가입이 완료된다(조건). <br> 
...
<설명> <br>
-회원가입 완료 후, <br>
로그인 버튼 클릭 시, 로그인 창으로, <br>
메인창 버튼 클릭 시, 메인 창으로 이동한다. <br>
...
열차표 조회 버튼을 클릭하면, 예매표 정보가 검색된다. <br>
...
<설명> <br>
-예매할 기차표 확인 후, 예매하기 버튼을 클릭하면, 선택한 기차표로 예매가 가능하다. <br>
...
-Spring mvc 패턴에서 Controller의 Mapping 방식 중 하나인 Post 방식(PostMapping)을 활용하여, <br>
로그인 유효성 검사를 진행한다. <br>
...
<설명> <br>
-로그인 실패 시, model.addAttribute를 활용하여, <br>
Console창에 실패 메세지가 출력되면서, 로그인 페이지에 머문다. <br>
...
<설명> <br>
-로그인 성공 시, model.addAttribute를 활용하여, <br>
Console창에 성공 메세지가 출력되면서, 메인 페이지로 리다이랙션 한다. <br>
- 로그인 실패 - 기능구현 상세 코드 <br>
https://github.com/hsy2493/HumanRail999/blob/master/htproject/src/main/java/htproject/t999/a01_Controller/A04_LoginController.java <br>
...
<설명> <br>
-Spring mvc 패턴에서 Dao(SQL)의 Mybatis에서 Select를 활용하여, <br>
...
<설명><br>
-Spring mvc 패턴에서 Dao(SQL)의 Mybatis에서 Insert를 활용하여, <br>
새로 입력한 회원정보가 Database에 저장(등록)된다.<br>
...
<설명> <br>
-Spring mvc 패턴에서 Service의 Model 데이터를 List로 가져오고, 이를 Dao(SQL)의 Mybatis에서 Select를 활용하여, <br>
...
- Back-end에서 Spring mvc을, Front-end에서 forEach문(JS)을 활용하여 연동 처리 후, <br> 
  기차표 예매 중 선택한 예매표 정보를 다른 페이지로 넘겨 해당 정보만 조회 기능구현이 가능함. <br>
- Bootstrap 템플릿 사용법을 터득함. <br>
- 프로젝트용 Dynamic Web Project 생성과 활용이 가능함.<br>
